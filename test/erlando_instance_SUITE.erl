...
test_state_t_metadata_is_retained(_Config) ->
    ...
    ?assert(
        lists:all(
            fun({1, Options}) -> is_map(Options) end,
            GenFunMetadata
        )
    ),
    ?assert(
...
test_monad_cont_dispatch_metadata(_Config) ->
    ...
    lists:foreach(
        fun(Instance) ->
            ...
            ?assertEqual(
                {maps:get(maps:get(type, Instance), Adapters), 2},
...
test_duplicate_capability_is_rejected(_Config) ->
    ...
    ?assertEqual(
        {error,
            {duplicate_erlando_instance_declaration, capabilities, [monad_runner, monad_runner]}},
        erlando_instance_macro:erlando_instance(
...
test_format_error(_Config) ->
    ?assert(
        is_list(
            erlando_instance_macro:format_error(
    ...
    ?assert(
        is_list(
            erlando_instance_macro:format_error(
                {duplicate_erlando_instance_declaration, types, [a, a]}
            )
        )
    ),
