...
'<*>'(STF, STA) ->
    state_t(
      fun(S) ->
              {AF, NS} = run_state(STF, S),
              {AA, NNS} = run_state(STA, NS),
              AB = applicative:'<*>'(AF, AA),
              {AB, NNS}
      end).
...
return(A) ->
    return(A, ?STATE_T_MONAD).

fail(E) ->
    fail(E, ?STATE_T_MONAD).
...
fail(E, {?MODULE, IM}) ->
    state_t(fun (_) -> monad_fail:fail(IM, E) end).
...
run_state(SM, S, {?MODULE, _IM} = _ST) -> run_state(SM, S).
