...
local(F, X) ->
    cont_t(fun (CC) ->
                   do([monad || 
                          R <- monad_reader:ask(),
                          monad_reader:local(F, run_cont(X, fun(A) -> monad_reader:local(fun(_) -> R end, CC(A)) end))
                      ])
           end).
...
lift_local(Ask, Local, F, X) ->    
    lift_local(Ask, Local, F, X, ?CONT_T_MONAD).
