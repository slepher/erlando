...
parse_transform(Forms, _Options) ->
    astranaut_traverse:map_with_state(fun walk/3, [], Forms).

walk({call, _Line, {atom, _Line1, do}, [{lc, _Line2, {AtomOrVar, _Line3, _MonadModule} = Monad, _Qs}]} = Node,
     MonadStack, #{step := pre})
...
walk({call, Line, {atom, _Line1, do},
      [{lc, _Line2, {AtomOrVar, _Line3, _MonadModule} = Monad, Qs}]}, [Monad|MonadStack], #{step := post}) 
  when AtomOrVar =:= atom orelse AtomOrVar =:= var orelse AtomOrVar =:= tuple ->
...
walk({call, Line, {atom, _Line1, return}, [Arg]}, [Monad|_T] = MonadStack, #{step := post}) ->
...
%%  'return' and 'fail' syntax detection and transformation:
walk({call, Line, {atom, _Line1, fail}, [Arg]}, [Monad|_T] = MonadStack, #{step := post}) ->
...
bind_syntax(Line, Pattern, Exprs, Monad) ->
    ...
    astranaut:merge_clauses([Clause1, Clause2]).
